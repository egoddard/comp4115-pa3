...
```SQL
MariaDB [employees]> source part_3_procedures.sql
Query OK, 0 rows affected (0.00 sec)

MariaDB [employees]> CALL get_name('Bouloucos');
+---------------+-----------+
...
###Average Salary
```SQL
MariaDB [employees]> CALL avg_salary('d009');
+-------------+
| AVG(salary) |
+-------------+
|  58770.3665 |
...
###Number of employees of a given gender
```SQL
MariaDB [employees]> CALL count_gender('M');
+---------------+
| COUNT(gender) |
+---------------+
|        179973 |
